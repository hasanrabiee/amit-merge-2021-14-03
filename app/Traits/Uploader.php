...
trait Uploader
{
    ...
    {
        $request->validate([
            $name => 'required|image|mimes:jpeg,png,jpg,gif|max:1024',
        ]);
        $file_path = \request($name)->getPathName();
        $imageName = bin2hex(random_bytes(32)) .'.'. \request($name)->getClientOriginalExtension();
    ...
    {
        $request->validate([
            'SiteIcon' => 'required|image|mimes:jpeg,png,jpg,gif|max:4096',
        ]);
